...
const BtmNav = () => {
  ...
  useEffect(() => {
    if (user && user[process.env.NEXT_PUBLIC_BASE_URL + 'type'].includes('sponsor'))
      setIsSponsor(true);
    if (user && user[process.env.NEXT_PUBLIC_BASE_URL + 'type'].includes('guest'))
      setIsGuest(true);
  }, [user]);
      ...
      <ul className={currentPage ? currentPage : 'home'}>
        ...
        {isSponsor && (
          <li className="sponsor-dashboard">
            <Link href="/dashboard/sponsor"><a>Sponsor Dashboard</a></Link>
          </li>
        )}

        {isGuest && (
          <li className="guest-dashboard">
            <Link href="/dashboard/guest"><a>Guest Dashboard</a></Link>
          </li>
        )}
