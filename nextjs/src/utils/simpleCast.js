...
export const getStatsForEpisode = async (
    episodeId,
    type = STAT_TYPES.DOWNLOADS
) => {
    const res = await fetch(
        `https://api.simplecast.com/analytics/${type}?episode=${episodeId}`,
        {
            headers: {
...
export const getStatsForEpisodes = async (episodeIds) => {
    const downloads = await Promise.all(
        episodeIds.map((id) => getStatsForEpisode(id, STAT_TYPES.downloads))
    );
